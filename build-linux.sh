...
build_stages() {
	...
	echo "FWOUT = $FWOUT"
    if [ ${#1} == 3 ];then
  		FWPOINT="${FWOUT:0:1}.${FWOUT:1}"
	elif [ ${#1} == 4 ];then
  		FWPOINT="${FWOUT:0:2}.${FWOUT:2}"
  	fi
	...
	if [ "${LOG}" == "log" ] || [ "${LOG}" == "log1" ]; then
		if [ "${LOG[0]}" == "log1" ]; then
		  	LOGFILE=$OUT/build-linux.log
		  	if [ -z "${LOG[1]}" ]; then
...
display_command_line() {
    echo "command line : '[firmware]' [log|log1] ['outpathfilename']"
	echo "firmware : fw version default: 'all'"
	echo "log/log1 : default: no log file"
	echo "log : enable log file"
    echo "log1 : enable log file, one log file by firmware"
    echo "'outpath' : out path for stages files default: 'Stages'"
}
...
do
	if [ $i == 0 ]; then
		FW=${ARGS[$i]}
		if [ ${ARGS[$i]} != "all" ]; then
			check_firmware "${ARGS[$i]}"
			echo "CheckFirmware ${FW} : '$?'"
			if [ $? == "0" ]; then
				echo "Firmware = ${FW}"
			else
	...
	elif [ -z "${LOGSET}" ] && [[ ${ARGS[$i]} == "log" || ${ARGS[$i]} == "log1" ]]; then
		set LOG=${ARGS[$i]}
		LOGSET="true"
	elif [ -z "${OUTSET}" ] && [[ ${ARGS[$i]} != "log"  && ${ARGS[$i]} != "log1" ]]; then
		set OUT=${ARGS[$i]}
		OUTSET="true"
	else
		echo "Error."
