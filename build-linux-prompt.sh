...
build_stages() {
	...
	if [[ "${LOG[0]}" == "-d" ]]; then
		if [[ "${LOG[1]}" == "-1" ]]; then
		  	LOGFILE=$OUT/build-linux-prompt.log
		  	if [ -z "${LOG[2]}" ]; then
...
# Prompt the user for the firmware version
echo "Select the firmware version for the build:"
echo " 0. FW=ALL"
for (( i=0;i<${#FWS[@]};i++ ));
do
	if [ ${#FWS[$i]} == 3 ];then
  		FW="${FWS[$i]:0:1}.${FWS[$i]:1}"
	elif [ ${#FWS[$i]} == 4 ];then
  		FW="${FWS[$i]:0:2}.${FWS[$i]:2}"
	fi
	if [ "$i" -lt 9 ];then
		echo " $(($i+1)). FW=${FW}"
	else
...
read -p "Enter your choice (0 to ${#FWS[@]}): " choice

# Determine the firmware version based on user input
if "$choice" < "0" || "$choice" > "${#FWS[@]}"; then
